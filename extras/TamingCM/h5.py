...
event_file_path = "zurich_city_10_a\events_left\events.h5"
flow_timestamp_path = "zurich_city_10_a/test_forward_flow_timestamps.csv"
event_set_t0 = 56538407054

true_file = h5py.File(event_file_path)
...
class H5Loader(BaseDataLoader):
    def __init__(self, config, shuffle=False, path_cache=""):
        ...
        flow_timestamp = np.genfromtxt(flow_timestamp_path, skip_header=1, delimiter=",")
        flowmaps = FlowMaps()
        for tms in flow_timestamp:
            flowmaps.names.append("flowmap" + str(int(tms[2]) // 2).zfill(9))
            flowmaps.ts_from.append(tms[0])
            flowmaps.ts_to.append(tms[1])
    ...
    def get_events(self, file, idx0, idx1):
        ...
        xs = true_file["events/x"][idx0:idx1]
        ys = true_file["events/y"][idx0:idx1]
        ts = true_file["events/t"][idx0:idx1]
        ps = true_file["events/p"][idx0:idx1]  # sequence starting at t0 = 0
    ...
    def find_ts_index(self, file, timestamp, dataset="events/t"):
        """
        ...
        timestamp -= event_set_t0
        return binary_search_array(true_file[dataset], timestamp)
