...
int main(int argc, char *argv[])
{
    ...
    std::string repackaged_events_path = "sequences/repackaged_events.h5";
    ...
    // Filter events
    HighFive::File file(repackaged_events_path, HighFive::File::ReadOnly);

    auto dts = file.getDataSet("events/t");
    auto dps = file.getDataSet("events/p");
    auto dys = file.getDataSet("events/y");
    auto dxs = file.getDataSet("events/x");
    const unsigned long long only = 8e5;
    unsigned long long idx = 0;
    std::vector<int> ts, ps, ys, xs;
    int N = dts.getDimensions()[0];
    int i = 0;
    while (true)
    {
        if (i == 0 || i == only) {
            ...
            ys.clear();
            xs.clear();
            unsigned long long cnt = std::min(only, N - idx);
            dts.select({ idx }, { cnt }).read(ts);
            dps.select({ idx }, { cnt }).read(ps);
            dys.select({ idx }, { cnt }).read(ys);
            dxs.select({ idx }, { cnt }).read(xs);
            idx += only;
            i = 0;
